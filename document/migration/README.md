...
| `POSTMAN_COLLECTION.json` | **Postman v2.1 Collection** — 100+ API requests across 19 folders (Auth, Products, Orders, Payments, Delivery, Email, SMS, OTP, Custom Orders, Invoices, CRM, etc.) |
| `API_TEST_GUIDE.md` | API verification checklist, security tests, parity matrix, automated health check script |
| `HOSTINGER_SETUP_GUIDE.md` | **Hostinger deployment guide** — Step-by-step setup, file structure, security checklist, troubleshooting |

## 📂 PHP Configuration Files (`php-config/`)

| File | Description |
|------|-------------|
| `database.php` | PDO MySQL connection config (auto-generated by installer) |
| `env.php` | Master environment config — JWT secret, encryption key, DB credentials |
| `email.php` | Email provider config — Hostinger SMTP, Resend, SendGrid, Mailgun + queue settings |
| `sms.php` | SMS gateway config — Twilio, BulkSMSBD, GreenWeb, SMSQ, Infobip, Nexmo + OTP settings |
| `payment.php` | Payment gateway config — SSLCommerz, bKash, Nagad, aamarPay, SurjoPay, COD |
| `delivery.php` | Delivery provider config — Pathao, Steadfast, RedX, PaperFly + shipping costs |
| `storage.php` | File storage config — bucket mapping, image processing, S3 optional |
| `htaccess.txt` | Apache .htaccess template — URL rewrite, security headers, GZIP, CORS |
...
### Recovery:
If auto-installer fails mid-way:
1. Delete `install.lock` if it exists
2. Re-upload `install.php` from `docs/migration/`
3. Run again — it detects existing tables and skips them
4. Each config file in `php-config/` can be manually copied to `config/`
...
| API Endpoints | 100+ | Full REST API documented in Postman collection |
| Config Files | 8 | Database, env, email, SMS, payment, delivery, storage, htaccess |
...
- [x] Postman collection with 100+ test requests
...
- [x] **PHP Config Package** — 8 config files for all subsystems
- [x] **Hostinger Setup Guide** — deployment, security, troubleshooting
- [x] Custom order form customization with CSV import/export
- [x] Invoice design customization with live preview
- [x] CRM data export (CSV, JSON, PDF)
- [x] Excel/CSV data download for all admin sections
