...
const HeroSection = () => {
  ...
  const isBannerOnly = !currentSlide.show_title && !currentSlide.show_description && 
                       !currentSlide.show_badge && !currentSlide.show_primary_button && 
                       !currentSlide.show_secondary_button;
  ...
  const getOverlayGradient = (position: string, opacity: number) => {
    ...
    switch (position) {
      case "right":
        return `linear-gradient(to left, hsl(var(--background) / ${opacityValue}), hsl(var(--background) / ${opacityValue * 0.8}), transparent)`;
      case "top":
        return `linear-gradient(to bottom, hsl(var(--background) / ${opacityValue}), hsl(var(--background) / ${opacityValue * 0.5}), transparent)`;
      case "bottom":
        return `linear-gradient(to top, hsl(var(--background) / ${opacityValue}), hsl(var(--background) / ${opacityValue * 0.5}), transparent)`;
      case "center":
        return `radial-gradient(circle, hsl(var(--background) / ${opacityValue * 0.7}), transparent 70%)`;
      default: // left
        return `linear-gradient(to right, hsl(var(--background) / ${opacityValue}), hsl(var(--background) / ${opacityValue * 0.8}), transparent)`;
    }
  };

  // Handle banner click for banner-only mode
  ...
  return (
    <section 
      className={`relative min-h-screen flex items-center justify-center overflow-hidden ${isBannerOnly ? "cursor-pointer" : ""}`}
    ...
    >
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="flex flex-wrap gap-4">
                  ...
                  {(currentSlide.show_secondary_button ?? true) && currentSlide.secondary_button_text && currentSlide.secondary_button_link && (
      ...
      {/* Scroll Indicator - hide in banner-only mode */}
      {!isBannerOnly && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
